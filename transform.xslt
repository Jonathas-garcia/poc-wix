    ...
    <xsl:copy>
        <xsl:apply-templates select="@*|node()"/>
            <xsl:element name="Property" namespace="http://schemas.microsoft.com/wix/2006/wi">
                <xsl:attribute name="Id">AMBIENTE</xsl:attribute>
                <xsl:element name="RegistrySearch" namespace="http://schemas.microsoft.com/wix/2006/wi">
                    ...
                    <xsl:attribute name="Name">AMBIENTE</xsl:attribute>
...
<xsl:template match='//wix:Wix/wix:Fragment/wix:ComponentGroup/wix:Component[wix:File[@Source and  (contains(@Source, "hk"))]]'>
  
  <xsl:copy>
  <xsl:apply-templates select="@*"/>    
      <xsl:element name="Condition" namespace="http://schemas.microsoft.com/wix/2006/wi">
        <xsl:text disable-output-escaping="yes">&lt;![CDATA[AMBIENTE="H"</xsl:text>
...
<xsl:template match='//wix:Wix/wix:Fragment/wix:ComponentGroup/wix:Component[wix:File[@Source and  (contains(@Source, "prod"))]]'>
  <xsl:copy>
      <xsl:apply-templates select="@*"/>
      <xsl:element name="Condition" namespace="http://schemas.microsoft.com/wix/2006/wi">
           <xsl:text disable-output-escaping="yes">&lt;![CDATA[AMBIENTE="P"</xsl:text>
        <xsl:text disable-output-escaping="yes">]]&gt;</xsl:text>
      </xsl:element>
       <xsl:apply-templates select="node()"/>    
  </xsl:copy>
